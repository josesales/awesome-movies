...
const MovieDetails = ({ location }: MovieDetailsProps) => {
  const history = useHistory();
  const basicMovie = location.state?.movie;
  ...
  const getVideoUrl = useCallback((videos: Result[]) => {
    let videoUrl = "";
    ...
    if (videos?.length > 0) {
      ...
      for (let index = videos.length - 1; index > 0; index--) {
        if (
          videos[index].official === true &&
          videos[index].key &&
          videos[index].site?.toLowerCase() === "youtube" &&
          (videos[index].type?.toLowerCase() === "trailer" ||
            videos[index].type?.toLowerCase() === "teaser")
        ) {
    ...
    const getMovieDetails = async () => {
      try {
        ...
        const data = await get(
          `movie/${basicMovie.id}?append_to_response=videos`
        );
          ...
          {loading ? (
            ...
            <React.Fragment>
              <h1 className="section-title">
                {movie.year ? `${movie.title} (${movie.year})` : movie.year}
              </h1>
